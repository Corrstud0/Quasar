...
execute as @e[type=marker, tag=datapipes_lib_power_generator, tag=!datapipes_lib_alreadyconnected, sort=nearest, scores={datapipes_lib_power_storage=1..}] at @s run function datapipes_lib:power_transfer/generator_connected with entity @s data.power
...
execute if score total_net_power datapipes_lib matches 1.. as @e[type=marker, tag=datapipes_lib_power_generator, tag=datapipes_lib_power_consumer, tag=datapipes_lib_current_generators, sort=nearest] unless score @s datapipes_lib_power_storage >= @s datapipes_lib_power_max at @s run function datapipes_lib:power_transfer/consumer_connected with entity @s data.power
...
tag @e[type=marker, tag=datapipes_lib_current_generators] remove datapipes_lib_current_generators
