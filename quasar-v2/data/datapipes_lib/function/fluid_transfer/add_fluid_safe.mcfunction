$execute store success score succeeded datapipes_lib run data modify entity @s data.fluids.tanks[$(tank)].type set value '$(type)'
$execute if score succeeded datapipes_lib matches 1 run data modify entity @s data.fluids.tanks[$(tank)].amount set value 0
