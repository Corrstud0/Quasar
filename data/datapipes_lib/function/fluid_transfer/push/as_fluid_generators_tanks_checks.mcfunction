...
$execute as @e[type=marker, tag=datapipes_lib_fluid_generator, tag=fluid_filter_$(type)] at @s run function datapipes_lib:fluid_transfer/push/fluid_generator_connected with storage datapipes_lib:temp array[0]

#put the fluids into consumers
$execute if score total_amount datapipes_lib matches 1.. as @e[type=marker, tag=fluid_filter_$(type), tag=!datapipes_lib_current_fluid_generators] at @s run function datapipes_lib:fluid_transfer/push/fluid_consumer_connected with storage datapipes_lib:temp array[0]

#priorize fluid tanks and such
$execute if score total_amount datapipes_lib matches 1.. as @e[type=marker, tag=fluid_filter_$(type), tag=fluid_priority_1] at @s run function datapipes_lib:fluid_transfer/push/fluid_consumer_connected with storage datapipes_lib:temp array[0]
