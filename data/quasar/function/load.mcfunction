...
data modify storage quasar:constants multiblock_core[{tier: "if score @s quasar.multiblock_tier matches 1", check: "if block ^ ^ ^ waxed_oxidized_copper if block ^ ^ ^-1 granite_slab[type=double]"}].function set value 'run function quasar:machines/gravshaft/setup'
